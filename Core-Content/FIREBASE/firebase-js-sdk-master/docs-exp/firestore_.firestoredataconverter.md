...
|  [toFirestore(modelObject, options)](./firestore_.firestoredataconverter.tofirestore_1.md) | Called by the Firestore SDK to convert a custom model object of type <code>T</code> into a plain JavaScript object (suitable for writing directly to the Firestore database). Used with [setDoc()](./firestore_.setdoc.md)<!-- -->,  and  with <code>merge:true</code> or <code>mergeFields</code>. |
